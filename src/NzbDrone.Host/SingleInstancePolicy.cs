...
namespace Bonarr.Host
{
    ...
    public class SingleInstancePolicy : ISingleInstancePolicy
    {
        ...
        public void PreventStartIfAlreadyRunning()
        {
            if (IsAlreadyRunning())
            {
                _logger.Warn("Another instance of Bonarr is already running.");
        ...
        public void WarnIfAlreadyRunning()
        {
            if (IsAlreadyRunning())
            {
                _logger.Debug("Another instance of Bonarr is already running.");
            }
        }
        ...
        private List<int> GetOtherNzbDroneProcessIds()
        {
            try
            {
                ...
                {
                    _logger.Info("{0} instance(s) of Bonarr are running", otherProcesses.Count);
                }
            ...
            {
                _logger.Warn(ex, "Failed to check for multiple instances of Bonarr.");
                return new List<int>();
            }
        }
    }
